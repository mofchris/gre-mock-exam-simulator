...
- The app open in a browser: either the hosted version
  (`https://mofchris.github.io/gre-mock-exam-simulator/`) or a local copy
  (see [How to run locally](howto-run-and-deploy.md)).
- Scratch paper. The real GRE gives you some; give yourself some.
...
## Step 2: Write the Issue essay (30 minutes)

Click **Start writing**. You get a prompt, task instructions, and a plain editor with a
live word count. Three things are deliberately missing, because the real test doesn't have
them either: spell-check, grammar-check, and access to your system clipboard (Cut / Copy /
...
The pattern for every section: directions screen → **Start section** → questions →
**Exit Section** when you're done (or time expires).

While inside a section you can move freely:
...
- In Quant sections, **Calc** opens the on-screen calculator. In Numeric Entry
  questions, click the answer box first, then the calculator's **Transfer Display**
  button copies its value in.
...
## Step 4: Watch the adaptivity work
...
trend live on the home screen under **Score History & Review**.
