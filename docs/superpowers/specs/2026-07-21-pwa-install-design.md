...
- **Generator + freshness test:** `sw.js` is generated and committed; `node --test` fails loudly if it is stale. Nothing runs that test automatically — there is no CI and no git hook (see §2.4) — so it converts a silent failure into an obvious one for whoever runs the suite. That is the trade taken to preserve the no-build deploy story.
...
- **activate:** delete this app's *other* caches — `keys.filter((k) => OWNED.test(k) && k !== CACHE)`, where `OWNED` is `/^gre-/` or `/^netplus-/` — then `clients.claim()`. The prefix filter is load-bearing, not tidiness: `CacheStorage` is scoped to the **origin**, not to the worker's scope, and all three PWAs share `mofchris.github.io`. An unfiltered sweep deletes the other apps' precaches, and they do not repopulate until their own `sw.js` changes bytes — so a user who opens Network+ permanently loses GRE's offline launch, and Metal's too. Harmless in Metal, which is the only PWA on its origin; live the moment a second app ships.
...
- **match options:** `{ cacheName: CACHE, ignoreSearch: true, ignoreVary: true }`, with a `.catch(() => undefined)` on both lookups. `cacheName` confines the search to our own precache, which matters once foreign caches survive — on a dev server two of these apps sit at scope `/` and their keys collide exactly. The `catch` is belt-and-braces: a missing `cacheName` resolves `undefined` per spec (verified in Chrome 150), but that was not verifiable on iOS Safari, and a rejection there would turn every in-scope request into a hard failure even online. Everything precached is a same-origin static asset keyed by URL alone, so `Vary` matching adds nothing but a failure mode for `crossorigin`-attributed loads.
...
- **Same-origin multi-app check — the one that has teeth.** Serve the *parent* directory of both repos on a single port so the apps sit on one origin at different paths (`/GRE/` and `/network-plus-mock-exam/`), mirroring GitHub Pages. Install one, then the other, and assert **both** caches survive with their own entry counts; then kill the server and cold-load the first app and confirm it still renders. Testing one app at a time on `localhost:8420` cannot detect cross-app cache deletion, because the second registration simply replaces the first — that blind spot let an origin-wide `caches.delete` sweep pass nine task reviews and two offline verifications.
