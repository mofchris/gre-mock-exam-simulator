...
- **Sign in:** username; 6-digit PIN (`inputmode="numeric"`, `maxlength="6"`, `--font-mono`, autocomplete off). **PIN entry renders as six segmented cells** (user-requested design, 2026-07-12): square boxes styled with `--r-input`/`--line`/`--paper`, the active cell highlighted with the `--accent` border treatment; entered digits mask to dots with the most recent digit briefly visible; a single invisible real input overlays the cells and drives them (preserves paste, numeric mobile keyboard, screen-reader semantics). Applies to every PIN field (sign-in, signup + confirm, reset + confirm). Turnstile container is empty until the server answers 403 `{needsCaptcha:true}` — then the widget renders and the user retries. The Turnstile `api.js` script is **lazy-loaded the first time any flow needs it, never for anonymous browsing** (parent spec §9.5)
